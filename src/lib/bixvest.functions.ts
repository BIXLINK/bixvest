...
export const activateMembership = createServerFn({ method: "POST" })
  ...
  .validator((input) => z.object({ code: z.string().trim().min(4) }).parse(input))
...
export const stakeVst = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input) => z.object({ level: z.number().int().min(1).max(10) }).parse(input))
  .handler(async ({ data, context }) => {
    const { userId } = context;
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
...
export const completeMission = createServerFn({ method: "POST" })
  .middleware([requireSupabaseAuth])
  .validator((input) => z.object({ mission_id: z.string() }).parse(input))
  .handler(async ({ data, context }) => {
    const { userId } = context;
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
