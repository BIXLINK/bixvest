  ...
  const { data: history = [] } = useQuery({
    ...
    queryFn: async () => {
      const { data } = await supabase.from("wallet_transactions").select("*")
        .eq("user_id", profile!.id).eq("type", "earn")
    ...
    <AppLayout>
      <div className="mx-auto max-w-7xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Participate. Earn VST. Build your activity profile.</p>
        </div>
        ...
        <Tabs defaultValue="tasks">
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
                {campaigns.map(c => (
                  <div key={c.id} className="rounded-xl border border-border bg-card p-5">
                    ...
                    <div className="mt-4 text-sm font-medium text-primary">+{fmt(Number(c.vst_reward))} VST</div>
                  </div>
                ))}
              </div>
            )}
          ...
          <TabsContent value="challenges" className="mt-6">
            {challenges.length === 0 ? (
              ...
              <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{challenges.map(t => <TaskCard key={t.id} task={t} />)}</div>
            )}
          </TabsContent>

          <TabsContent value="history" className="mt-6">
            <div className="rounded-xl border border-border bg-card">
              {history.length === 0 ? <EmptyState text="No VST earned yet." /> : (
  ...
  return (
    <>
      <div className="flex flex-col rounded-xl border border-border bg-card p-5">
        <Badge variant="outline" className="w-fit capitalize">{task.type}</Badge>
        ...
        <div className="mt-4 flex items-center justify-between">
          <div className="text-sm font-medium text-primary">+{fmt(Number(task.vst_reward))} VST</div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            ...
            <Button onClick={send} disabled={loading}>{loading ? "Submitting..." : "Submit for review"}</Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
    </>
...
function EmptyState({ text }: { text: string }) {
  return <div className="rounded-xl border border-dashed border-border p-12 text-center text-sm text-muted-foreground">{text}</div>;
}
