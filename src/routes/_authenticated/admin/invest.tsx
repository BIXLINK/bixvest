...
function InvestAdmin() {
  ...
  const [form, setForm] = useState<any>({ name: "", description: "", min_amount: 100, apr: 5, status: "draft" });

  const { data: products = [] } = useQuery({
    queryKey: ["invest-products-all"],
    queryFn: async () => {
      const { data } = await (supabase as any).from("invest_products").select("*").order("created_at", { ascending: false });
      return data ?? [];
    },
  });
  ...
  async function save() {
    try {
      await upsert({ data: { ...form, min_amount: Number(form.min_amount), apr: Number(form.apr) } });
      toast.success("Saved");
      setOpen(false);
      ...
      <div className="mx-auto max-w-5xl space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Manage products shown in the Invest module.</p>
          </div>
          <Button onClick={() => { setForm({ name: "", description: "", min_amount: 100, apr: 5, status: "draft" }); setOpen(true); }}>New Product</Button>
        </div>

        <div className="rounded-xl border border-border bg-card overflow-x-auto">
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-xs uppercase text-muted-foreground">
              <tr><th className="px-3 py-2 text-left">Name</th><th className="px-3 py-2">Min</th><th className="px-3 py-2">APR</th><th className="px-3 py-2">Status</th><th /></tr>
            </thead>
            <tbody className="divide-y divide-border">
              {products.length === 0 ? (
                ...
                <tr key={p.id}>
                  ...
                  <td className="px-3 py-2 text-center"><span className="rounded-full bg-muted px-2 py-0.5 text-xs">{p.status}</span></td>
          ...
          <DialogContent>
            ...
            <div className="space-y-3">
              ...
              <div className="grid grid-cols-3 gap-2">
                <div><label className="text-xs">Min Amount</label><Input type="number" value={form.min_amount} onChange={e => setForm({ ...form, min_amount: e.target.value })} /></div>
                <div><label className="text-xs">APR %</label><Input type="number" value={form.apr} onChange={e => setForm({ ...form, apr: e.target.value })} /></div>
                <div>
