...
function StakingPage() {
  ...
  useEffect(() => {
    ...
    levels.forEach((l) => {
      init[l.level] = String(l.vst_required);
    });
    setEdits(init);
  }, [levels.length]);
      ...
      <div className="space-y-4">
        ...
        <div className="rounded-xl border border-border bg-card divide-y divide-border">
          {levels.map((l) => (
            <div
              key={l.level}
              className="grid grid-cols-[60px_1fr] items-center gap-3 px-4 py-3 sm:grid-cols-[80px_1fr_200px_auto] sm:px-5"
            >
              ...
              <Input
                ...
                value={edits[l.level] ?? ""}
                onChange={(e) => setEdits((s) => ({ ...s, [l.level]: e.target.value }))}
              />
              <Button size="sm" className="col-span-2 sm:col-span-1" onClick={() => save(l.level)}>
                Save
              </Button>
            </div>
          ))}
