...
function TasksPage() {
    ...
    queryFn: async () =>
      (
        await supabase
          ...
          .select(
            "*, tasks(title, vst_reward), profiles!task_submissions_user_id_fkey(email, full_name)",
          )
  ...
  async function create() {
    const { error } = await supabase
      .from("tasks")
      .insert({ title, description: desc, type: type as any, vst_reward: Number(reward) });
                  ...
                  {submissions.map((s: any) => (
                    ...
                    >
                      <div>
                        ...
                        {s.proof && (
                          <div className="mt-1 text-xs bg-muted rounded p-2 break-words">
                            {s.proof}
                          </div>
                        )}
                      </div>
