...
function CodesPage() {
  ...
  const { data: codes = [] } = useQuery({
    ...
    queryFn: async () => (await supabase.from("activation_codes").select("*").order("created_at", { ascending: false }).limit(500)).data ?? [],
  });
        ...
        <div className="rounded-xl border border-border bg-card p-5">
          ...
          <div className="grid gap-3 sm:grid-cols-[140px_1fr_auto]">
            ...
            <div className="flex items-end">
              <Button onClick={gen} disabled={busy}>{busy ? "Generating..." : "Generate"}</Button>
            </div>
          </div>
        </div>
              ...
              {codes.map(c => (
                <tr key={c.id}>
                  ...
                  <td className="px-4 py-3">
                    <span className={`text-xs uppercase tracking-wider ${c.status === "used" ? "text-success" : c.status === "disabled" ? "text-destructive" : "text-warning"}`}>
                      {c.status}
                    </span>
                  </td>
                  ...
                  <td className="px-4 py-3 text-right space-x-2">
                    ...
                    {c.status === "unused" && (
                      <Button size="icon" variant="ghost" onClick={async () => {
                        try { await disable({ data: { id: c.id } }); toast.success("Disabled"); qc.invalidateQueries({ queryKey: ["activation-codes"] }); }
                        catch (err) { toast.error((err as Error).message); }
                      }}>
