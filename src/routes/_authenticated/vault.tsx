    ...
    <AppLayout>
      <div className="mx-auto max-w-7xl space-y-6">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">Stake VST into progressive growth levels.</p>
        </div>
        ...
        <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5">
          {levels.map(lvl => {
            ...
            return (
              <div key={lvl.level} className={`rounded-xl border p-5 ${achieved ? "border-primary/40 bg-gradient-card text-white" : next ? "border-primary/30 bg-card" : "border-border bg-card"}`}>
                <div className="flex items-center justify-between">
                  <Vault className={`h-4 w-4 ${achieved ? "text-white/70" : "text-muted-foreground"}`} />
                  {achieved && <Check className="h-4 w-4 text-primary" />}
                </div>
                <div className={`mt-3 text-xs uppercase tracking-wider ${achieved ? "text-white/60" : "text-muted-foreground"}`}>Level {lvl.level}</div>
                <div className="font-display text-xl font-bold">{lvl.name}</div>
                <div className={`mt-1 text-sm ${achieved ? "text-white/80" : "text-muted-foreground"}`}>{fmt(Number(lvl.vst_required))} VST</div>
                <div className="mt-4">
                  {achieved ? (
        ...
        <div className="rounded-xl border border-border bg-card">
          <div className="border-b border-border px-6 py-4 font-display font-semibold">Stake history</div>
          {stakes.length === 0 ? (
...
function Stat({ label, value, hero }: { label: string; value: string; hero?: boolean }) {
  return (
    <div className={`rounded-xl border p-5 ${hero ? "border-transparent bg-gradient-emerald text-primary-foreground shadow-glow" : "border-border bg-card"}`}>
      <div className={`text-xs uppercase tracking-wider ${hero ? "text-white/70" : "text-muted-foreground"}`}>{label}</div>
